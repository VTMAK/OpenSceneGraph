   ...
   */
   ...
   protected:
      ...
      virtual void readRecord(RecordInputStream& in, Document& document)
      {
         ...
         // BEGIN VRV_PATCH
         // For CDB, cultural footprints are hidden
         // This was added for ST's old CDB terrain
         if (document.getCdb() && isFootprint())
             _geode->setNodeMask(0);
         // END VRV_PATCH
         ...
         // BEGIN VRV_PATCH - Defect 61992 - WorldFlatEarthNoElevation doesn't show anything
         else
         {
         ...
         if (isLit() && materialIndex >= 0)
         {
            osg::ExtendedMaterial* extendedMaterial = document.getOrCreateExtendedMaterialPool()->get(materialIndex);
            if (extendedMaterial)
            {
               applyExtendedMaterial(extendedMaterial, document, stateset);
            }
         }
         ...
         {
            stateset->setAttributeAndModes(document.getOrCreateCullFaceAttribute(), osg::StateAttribute::ON);
            break;
         }
         ...
         //  See: https://jira.mak.com/browse/PS-3980
         // 
      ...
      virtual void dispose(Document& document)
      {
         if (_geode.valid())
         {
            ...
            if (document.getUseTextureAlphaForTransparancyBinning())
            {
               for (unsigned int i = 0; i<stateset->getTextureAttributeList().size(); ++i)
               {
                  ...
                  if (texture)
                  {
                     ...
                     if (image)
                     {
                        std::string simpleName = osgDB::getSimpleFileName(image->getFileName());
                        if (document.getForcedOpaqueImages().find(simpleName)
                           != document.getForcedOpaqueImages().end())
                        {
                           forceOpaque = true;
                           //Break out of loop
                           break;
            ...
            if (!forceOpaque)
            {
               ...
               {
                  stateset->setAttributeAndModes(document.getOrCreateBlendFuncAttribute(), osg::StateAttribute::ON);
      ...
      void applyExtendedMaterial(osg::ExtendedMaterial* material, Document& document, osg::StateSet* stateSet)
      {
         const osg::ExtendedMaterial::EffectTextureIndexMap& indexMap = material->getEffectTextureIndexMap();
         //start from 1, 0 is alwayes reserved for base texture
         int textureUnit = 1;
         for (int i = osg::ExtendedMaterial::AMBIENT_LAYER; i <= osg::ExtendedMaterial::PHYSICAL_MAP_LAYER; ++i)
         {
            if (indexMap.at(i) >= 0)
            {
               int textureIndex = indexMap.at(i);
               ...
               {
                  osg::Texture* texture = dynamic_cast<osg::Texture*>(texturePoolStateset
                     ->getTextureAttribute(0, osg::StateAttribute::TEXTURE));
                  std::string textureName = getTextureFilePath(texture);
                  material->setEffectTextureName((osg::ExtendedMaterial::EffectTextureLayer)i, textureName);
                  //For diffuse texture, it's tricky. If there is no base texture in the face attribute slot,
                  //we will use the diffuse texture in the extended material to use as base texture, otherwise
                  //we will use the one in the face attribute , ignore the one in extended material.
                  if (i == osg::ExtendedMaterial::DIFFUSE_LAYER)
                  {
                     //No base texutre in the slot 0, which means we don't have any base texture in the face attribute
                     if (!stateSet->getTextureAttribute(0, osg::StateAttribute::TEXTURE))
                     {
                        //use the one from the extened material
         ...
         document.getExtendedMaterialPool()->setFirstTextureUnitAfterEffectTexture(textureUnit);
         stateSet->setAttributeAndModes(material);
      }

      std::string getTextureFilePath(osg::Texture* texture)
      {
         std::string baseTextureLocation;

         osg::Image *baseImage = texture->getImage(0);
         if (baseImage)
         {
            return baseImage->getFileName();
         }
         else
         {
            baseTextureLocation = texture->getName();
         }
