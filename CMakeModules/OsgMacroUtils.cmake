    ...
    IF(MSVC)
        ...
        ELSE(NOT CMAKE24)
            ...
            IF(OSG_MSVC_VERSIONED_DLL)
                ...
                IF(NOT MSVC_IDE)
                    SET_TARGET_PROPERTIES(${TARGET_TARGETNAME} PROPERTIES PREFIX "../bin${libext}/${OSG_PLUGINS}/")
                ELSE(NOT MSVC_IDE)
                    SET_TARGET_PROPERTIES(${TARGET_TARGETNAME} PROPERTIES PREFIX "../../bin${libext}/${OSG_PLUGINS}/" IMPORT_PREFIX "../")
                ENDIF(NOT MSVC_IDE)
...
#the installation path are differentiated for win32 that install in bib versus other architecture that install in lib${libext}/${OSG_PLUGINS}
    IF(WIN32)
        INSTALL(TARGETS ${TARGET_TARGETNAME}
            RUNTIME DESTINATION bin${libext} COMPONENT ${PACKAGE_COMPONENT}
            ARCHIVE DESTINATION lib${libext}/${OSG_PLUGINS} COMPONENT libopenscenegraph-dev
            LIBRARY DESTINATION bin${libext}/${OSG_PLUGINS} COMPONENT ${PACKAGE_COMPONENT})
        IF(MSVC AND DYNAMIC_OPENSCENEGRAPH)
            INSTALL(FILES ${OUTPUT_BINDIR}/${OSG_PLUGINS}/${TARGET_TARGETNAME}${CMAKE_RELWITHDEBINFO_POSTFIX}.pdb DESTINATION bin${libext}/${OSG_PLUGINS} COMPONENT ${PACKAGE_COMPONENT} CONFIGURATIONS RelWithDebInfo)
            INSTALL(FILES ${OUTPUT_BINDIR}/${OSG_PLUGINS}/${TARGET_TARGETNAME}${CMAKE_UNOPTIMIZED_POSTFIX}.pdb DESTINATION bin${libext}/${OSG_PLUGINS} COMPONENT ${PACKAGE_COMPONENT} CONFIGURATIONS Unoptimized)
            INSTALL(FILES ${OUTPUT_BINDIR}/${OSG_PLUGINS}/${TARGET_TARGETNAME}${CMAKE_DEBUG_POSTFIX}.pdb DESTINATION bin${libext}/${OSG_PLUGINS} COMPONENT ${PACKAGE_COMPONENT} CONFIGURATIONS Debug)
        ENDIF(MSVC AND DYNAMIC_OPENSCENEGRAPH)
    ELSE(WIN32)
        INSTALL(TARGETS ${TARGET_TARGETNAME}
            RUNTIME DESTINATION bin${libext} COMPONENT ${PACKAGE_COMPONENT}
            ARCHIVE DESTINATION lib${libext}/${OSG_PLUGINS} COMPONENT libopenscenegraph-dev
            LIBRARY DESTINATION lib${libext}/${OSG_PLUGINS} COMPONENT ${PACKAGE_COMPONENT})
    ENDIF(WIN32)
...
MACRO(SETUP_APPLICATION APPLICATION_NAME)
        ...
        IF(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION bin${libext} BUNDLE DESTINATION bin)
        ELSE(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION bin${libext} COMPONENT openscenegraph  )
            IF(MSVC)
                INSTALL(FILES ${OUTPUT_BINDIR}/${TARGET_NAME}${CMAKE_RELWITHDEBINFO_POSTFIX}.pdb DESTINATION bin${libext} COMPONENT openscenegraph CONFIGURATIONS RelWithDebInfo)
                INSTALL(FILES ${OUTPUT_BINDIR}/${TARGET_NAME}${CMAKE_UNOPTIMIZED_POSTFIX}.pdb DESTINATION bin${libext} COMPONENT openscenegraph CONFIGURATIONS Unoptimized)
                INSTALL(FILES ${OUTPUT_BINDIR}/${TARGET_NAME}${CMAKE_DEBUG_POSTFIX}.pdb DESTINATION bin${libext} COMPONENT openscenegraph CONFIGURATIONS Debug)
            ENDIF(MSVC)
        ENDIF(APPLE)
...
MACRO(SETUP_EXAMPLE EXAMPLE_NAME)
        ...
        IF(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION share/OpenSceneGraph/bin${libext} BUNDLE DESTINATION share/OpenSceneGraph/bin${libext} )
        ELSE(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION share/OpenSceneGraph/bin${libext} COMPONENT openscenegraph-examples )
            IF(MSVC)
                INSTALL(FILES ${OUTPUT_BINDIR}/${TARGET_NAME}${CMAKE_RELWITHDEBINFO_POSTFIX}.pdb DESTINATION share/OpenSceneGraph/bin${libext} COMPONENT openscenegraph-examples CONFIGURATIONS RelWithDebInfo)
                INSTALL(FILES ${OUTPUT_BINDIR}/${TARGET_NAME}${CMAKE_UNOPTIMIZED_POSTFIX}.pdb DESTINATION share/OpenSceneGraph/bin${libext} COMPONENT openscenegraph-examples CONFIGURATIONS Unoptimized)
                INSTALL(FILES ${OUTPUT_BINDIR}/${TARGET_NAME}${CMAKE_DEBUG_POSTFIX}.pdb DESTINATION share/OpenSceneGraph/bin${libext} COMPONENT openscenegraph-examples CONFIGURATIONS Debug)
            ENDIF(MSVC)
        ENDIF(APPLE)
